...
func (s *fortinetSession) connectPPPTLS() (pppSessionCarrier, error) {
	...
	return pppSessionCarrier{
		connection:    &fortinetTLSConn{Conn: tlsConnection},
...
func (c *fortinetTLSConn) Read(content []byte) (int, error) {
	...
	for {
		...
		if len(c.pending) >= len(prefix) {
			...
			if responseErr != nil {
				return 0, markTerminal(E.Cause(responseErr, "parse Fortinet TLS tunnel failure response"))
			}
			if statusCode >= 400 && statusCode <= 499 {
				return 0, ErrSessionRejected
			}
			return 0, markTerminal(E.New("Fortinet response-less TLS switch unexpectedly returned HTTP status ", statusCode))
		}
		if readErr != nil {
			return 0, markTerminal(E.Cause(readErr, "read partial Fortinet TLS response prefix"))
		}
	}
}
