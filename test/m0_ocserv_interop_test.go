...
readCSTPDPD:
	for {
		...
		case anyConnectPacketData:
			// ocserv may emit unsolicited IPv6 control traffic before the DPD echo.
		default:
...
readCSTPEcho:
	for {
		...
		switch packetType {
		case anyConnectPacketData:
			if len(packetPayload) > 0 && packetPayload[0]>>4 == 6 {
				// ocserv may emit unsolicited IPv6 control traffic before the
				// requested IPv4 echo reply.
				continue
			}
...
readDTLSDPD:
	for {
		...
		case anyConnectPacketDPDResponse:
			...
			break readDTLSDPD
		case anyConnectPacketData:
			if n > 1 && dtlsBuffer[1]>>4 == 6 {
				continue
			}
			t.Fatalf("unexpected DTLS data packet while waiting for DPD: %x", dtlsBuffer[:n])
		case anyConnectPacketKeepalive:
		default:
...
func startOcservContainer(t *testing.T, ctx context.Context) ocservContainer {
	...
	waitForOcservReady(t, ctx, containerName)
	waitForTCP(t, ctx, tcpAddress)
	return ocservContainer{name: containerName, tcpAddress: tcpAddress, udpAddress: udpAddress}
}

func waitForOcservReady(t *testing.T, ctx context.Context, containerName string) {
	t.Helper()
	waitContext, cancelWait := context.WithTimeout(ctx, 20*time.Second)
	defer cancelWait()
	for {
		logs, logsErr := dockerOutput(waitContext, "logs", "--tail", "100", containerName)
		if logsErr == nil && strings.Contains(logs, "initialized OpenConnect VPN Server") {
			return
		}
		select {
		case <-waitContext.Done():
			t.Fatal(E.Cause(waitContext.Err(), "wait for ocserv initialization"))
