...
#
# Derived from OpenConnect commit 2035601b64a5360a46d18e08937e7f654b3230f2,
# tests/fake-cisco-server.py. The expected-hash, expected-certificate-count,
# independent TLS client CA, success-marker, and post-MCA action/CSD controls
# are test extensions.
...
def get_certs(certs_der):
    ...
    for cert in extracerts:
        subject = cert.get_subject().der()
        assert subject not in issuers
        issuers[subject] = cert
...
def host_scan_action_fragment(action_url):
    return dedent('''
        <host-scan>
          <host-scan-ticket>host-scan-ticket</host-scan-ticket>
          <host-scan-token>original-host-scan-token</host-scan-token>
        ...
        <auth id="main"><form method="POST" action="{}"/></auth>'''.format(action_url))


def response_with_action(response, action_url):
    return response.replace('</config-auth>', host_scan_action_fragment(action_url) + '\n</config-auth>')
...
def initial_request(dict_req):
    ...
    return app.config['initial_response']
...
def auth_reply(dict_req):
    ...
    assert algo == app.config['expected_hash']

    certs = get_certs(certs_pkcs7)
    assert len(certs) == app.config['expected_certificate_count']
    ...
    if USE_CRYPTOGRAPHY:
        ...
        if isinstance(pub_key, ec.EllipticCurvePublicKey):
            pub_key.verify(signature, app.config['initial_response'].encode(), ec.ECDSA(hash_algs[algo]))
        else:
            pub_key.verify(signature, app.config['initial_response'].encode(), padding.PKCS1v15(), hash_algs[algo])
    else:
        verify(certs[0], signature, app.config['initial_response'].encode(), algo)

    print('MCA_AUTH_SUCCESS hash=%s certificates=%d' % (algo, len(certs)), flush=True)
    return app.config['authentication_response']
...
def main(args):
    ...
    if args.require_client_cert:
        if not args.client_cafile:
            raise ValueError('--require-client-cert requires --client-cafile')
        context.load_verify_locations(cafile=args.client_cafile)
        context.verify_mode = ssl.CERT_REQUIRED

    # Read cafile, parsing each certificate found
    ca_certs = []
    if args.cafile:
        with open(args.cafile, 'r') as file:
            root_cas_pem = file.read()
    ...
    app.config['expected_hash'] = args.expected_hash
    app.config['expected_certificate_count'] = args.expected_certificate_count
    app.config['initial_response'] = INITIAL_RESPONSE
    app.config['authentication_response'] = AUTH_COMPLETE_RESPONSE
    if args.post_mca_action_url:
        app.config['initial_response'] = response_with_action(INITIAL_RESPONSE, args.post_mca_action_url)
        app.config['authentication_response'] = response_with_action(
            dedent('''
                <?xml version="1.0" encoding="UTF-8"?>
                <config-auth client="vpn" type="auth-request" aggregate-auth-version="2">
                </config-auth>'''),
            args.post_mca_action_url)
    app.run(host=args.host, port=args.port, debug=True, use_reloader=False, ssl_context=context)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--client-cafile', help='Path to TLS machine certificate CA file')
    parser.add_argument('--expected-hash', choices=ALLOWED_HASH_ALGORITHMS, default='sha512')
    parser.add_argument('--expected-certificate-count', type=int, default=2)
    parser.add_argument('--require-client-cert', action='store_true')
    parser.add_argument('--post-mca-action-url')
