...
func (p *anyConnectCSDInteropPeer) validateReport(report []byte) error {
	...
	for name, value := range parsedReport.values {
		...
		handled, portErr := validateAnyConnectCSDReportedPort(name, value)
		if portErr != nil {
			return portErr
		}
		if handled {
...
func validateAnyConnectCSDReportedPort(
	...
	for _, prefix := range []string{
		...
		"endpoint.device.tcp4port",
		"endpoint.device.tcp6port",
	} {
		...
		if parseErr != nil || port == 0 || value != "true" {
			return true, E.New("CSD report contained an invalid listening port: ", name, "=", value)
		}
		return true, nil
	}
	return false, nil
